...
### Fixed
- **`llm-wiki` bootstrap 플레이스홀더 체크리스트 (2026-07-21)**: TASK-M 실전 검증의 결함 ⑤.
  절차 A 5단계와 walkthrough §4 가 SCHEMA 의 §1·§3·§4·§5 만 언급해, 제목 `<Wiki Name>` 과
  §2·§6·§7 의 자리표시자는 지시 없이 방치됐다.
  - 조사 결과 템플릿의 플레이스홀더 **37개(32줄)** 는 두 종류였다 — *치환할 인스턴스 값*
    (제목/머리말/§1/§2/§6/§7) 과 *그대로 둘 형식 예시* (§3 파일명 규칙, §4 frontmatter 키,
    §5 링크 형식). walkthrough 가 이 **구분을 알려주지 않는 것**이 결함의 실체였다.
  - `bootstrap-walkthrough.md` §4.1 신설 — (A) 치환 / (B) 보존 2표. 기존 도메인 적응 내용은
    §4.2 로 이동. §6 자주 하는 실수에 2행 추가 (자리표시자 방치 / 형식 예시까지 치환).
  - `SKILL.md` §Procedure.A.5 가 §4.1 을 가리키도록 보강.
  - 검증: 체크리스트가 37개 플레이스홀더 **전부를 덮는지** 기계 확인 (미포함 0). TASK-M 에서
    만든 SCHEMA 에 (A) 방치 0건 / (B) 보존 7건 — 분류가 실제 작업과 일치함을 확인.
