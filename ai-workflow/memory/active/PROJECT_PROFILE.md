...
- **frontmatter 정책 (2026-07-10 결정)**: agentskills.io canonical 40+ 제품 (Claude Code, Cursor,
  Cline, VS Code, OpenAI Codex, GitHub Copilot, Gemini CLI, Roo Code, Spring AI 등) 의 base
  shape 정합 + Claude-Code-flavored extension 허용. **현재 상태 유지** — `metadata.harness_compat`,
  `metadata.category`, `metadata.hermes.*`, top-level `when_to_use` / `version` / `author` /
  `license` / `platforms` 모두 유지. **향후 migration 방향**: hybrid (depth-2 nested) —
  `metadata.claude_code.{when_to_use,version,...,harness_compat,category,tags,related_skills}`.
  이 migration 은 `scripts/skill-lint` + `scripts/skill-discover` 의 YAML mini-parser 를
  recursive depth-n 으로 확장할 때 *동시에* 진행 (별도 task — 본 세션에서는 *코드 변경 없음*).
  결정 근거: 2차 deep-research (TASK-PI-Followup, 2026-07-10) Axes 3 AI agent skills 의
  1차 4 open question 중 마지막 1개 해소.
